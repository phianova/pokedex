...
const evolutionData = async (req, res, next) => {
    const species = req.params.species; 
    const speciesData = await axios.get(`https://pokeapi.co/api/v2/pokemon-species/${species}`);
...
exports.addPokemon = async (req, res, next) => {
    if (pokemon.length >= 6) return next(createError(400, 'Pokemon Limit Reached - please swap or delete'));
    ...
    if (allData.evolutionResponse.chain.evolves_to.length > 0) {
        if (currentSpecies === allData.evolutionResponse.chain.species.name){
            ...
            if (allData.evolutionResponse.chain.evolves_to[0].evolves_to.length > 0){
                if (currentSpecies === allData.evolutionResponse.chain.evolves_to[0].species.name){
                    nextSpecies = allData.evolutionResponse.chain.evolves_to[0].evolves_to[0].species.name;
                } else {
                    if (allData.evolutionResponse.chain.evolves_to[0].evolves_to[0].evolves_to.length > 0){
                        if (currentSpecies === allData.evolutionResponse.chain.evolves_to[0].evolves_to[0].species.name){
                        nextSpecies = allData.evolutionResponse.chain.evolves_to[0].evolves_to[0].evolves_to[0].species.name;
                        }
                    }
                }
            } 
